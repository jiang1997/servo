...
pub fn send_response_to_devtools(
    ...
    let security_info: Option<TlsSecurityInfo>: Option<TlsSecurityInfo> = meta.tls_security_info.clone();
    ...
    if let Some(info) = security_info {
        send_security_info_to_devtools(Some(info), request, context.devtools_chan.clone());
    }
}
...
pub fn send_security_info_to_devtools(
    ...
    if security_info.is_none() {
        return;
    }

    if let (Some(devtools_chan), Some(webview_id)) =
        (devtools_chan, request.target_webview_id.map(|id| id.into()))
    {
